...
describe('add important values filter', () => {
  ...
  const objectTypeWithProjectsScope = new ObjectType({
    elemID: new ElemID('jira', 'objectTypeWithProjectsScope'),
    fields: {
      projectsScope: { refType: new ListType(BuiltinTypes.STRING) },
    },
  })
  ...
  describe('onFetch', () => {
    it('should add important values annotation correctly', async () => {
      await filter.onFetch([automationType, fieldType, projectType, scheduledJobType, objectTypeWithProjectsScope])
      ...
      expect(objectTypeWithProjectsScope.annotations[CORE_ANNOTATIONS.IMPORTANT_VALUES]).toEqual([
